...
#' @param ignore_colnames the column names to ignore when comparing \code{sufl_data} with the data in the
...
#' @param overwrite_na_to_missing if sufl_data contains an NA, should this data be ignored or should
...
api_check_data = function(sufl_data, ignore_colnames = c("first_name", "last_name"),
  ...
  if(length(data_from_app) == 0){
    ...
    # if no_action_test3 is FALSE but overwrite_na_to_missing is FALSE, then bioscreen
    # should not be updated to missing when sufl_data is missing so no_action_test3 should be TRUE
