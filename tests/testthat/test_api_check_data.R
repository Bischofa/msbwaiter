library(msbwaiter)
context("Testing api_check function")
...
# tests
test_that("api_check returns 'no action' when appropriate", {
  ignore_colnames = c("first_name", "last_name")

  # if the data is identical to that in the bioscreen, 'no action' should be returned
  action = api_check(sufl_data = sufl_data, ignore_colnames = ignore_colnames, endpoint = endpoint,
                          base_url = base_url, verbose_b = verbose_b)
...
test_that("api_check returns 'update' when appropriate", {
  ignore_colnames = c("first_name", "last_name")
  sufl_data = api_get(source_id = source_id, external_identifier = external_identifier, endpoint = endpoint,
                      base_url = base_url, verbose_b = verbose_b)
...
test_that("api_check returns 'create' when appropriate", {
  endpoint = "subjects"
  ignore_colnames = c("first_name", "last_name")
